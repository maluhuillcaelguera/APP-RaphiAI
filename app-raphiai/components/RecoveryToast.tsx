...
type RecoveryToastContextValue = {
  show: (message: string, type?: ToastType, durationMs?: number) => void;
};

const RecoveryToastContext = React.createContext<RecoveryToastContextValue | undefined>(undefined);

export function useRecoveryToast(): RecoveryToastContextValue {
  const ctx = React.useContext(RecoveryToastContext);
  if (!ctx) throw new Error('useRecoveryToast must be used within RecoveryToastProvider');
  return ctx;
}

export function RecoveryToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const show = React.useCallback((msg: string, t: ToastType = 'info', durationMs = 2800) => {
  ...
  return (
    <RecoveryToastContext.Provider value={value}>
      {children}
      {visible && (
